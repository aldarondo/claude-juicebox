...
### 🔴 Blocked
- **VPN profile still pins one specific server — this will recur** — the tunnel is fixed (below) but now points at `us13142.nordvpn.com`. When NordVPN retires *that* server the same silent black-hole returns, because `redirect-gateway=yes` sends all NAS outbound through `tun0` and `reconnect=yes` cannot help when the endpoint no longer exists. Durable fix worth doing: `remote us.nordvpn.com 1194` with `verify-x509-name nordvpn.com name-suffix` (instead of a specific CN), plus a watchdog that curls `api.ipify.org` and force-reconnects when egress is not a NordVPN IP.
