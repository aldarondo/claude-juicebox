...
### 🔴 Blocked
- **⚠️ NAS NordVPN tunnel is down and cannot be redialed — NAS is currently on direct, un-VPN'd internet (2026-08-08)** — found while trying to `docker compose pull`. The tunnel was in the known zombie state ([[nordvpn-zombie-tunnel-recovery]]): `synovpnc get_conn` reported connected with 2,245,236 s (~26 days) uptime, but **zero** live `openvpn` processes and the `tun0` default route black-holing all outbound traffic (`curl https://ghcr.io/v2/` and `https://api.cloudflare.com` both timed out at 12 s). This single fault also explains both CI deploy failures — the NAS-side `cloudflared` needs outbound to hold the tunnel up, hence `websocket: bad handshake`. `synovpnc kill_client` restored internet immediately via the LAN gateway (ghcr.io → 401 in 0.35 s), but the documented redial (`cp vpnc_last_connect vpnc_connecting` + `synovpnc connect --id=o1774644869`) fails with `TLS key negotiation failed to occur within 60 seconds` / `TLS handshake failed`. **Root cause: the profile pins `remote 91.196.220.56 1194` — a raw IP, not a hostname.** NordVPN rotates and decommissions servers, so that endpoint is simply gone and no redial can succeed. Needs a decision from Charles: repoint the profile at a current NordVPN endpoint (prefer a hostname so rotation stops breaking it) and re-import credentials. **Until then the NAS has working but un-VPN'd internet — relevant if the VPN was shielding the media stack.** Charging is unaffected (all LAN).
